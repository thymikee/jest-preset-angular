...
function tryLoadV9Transformer(program: Program): TransformerFactory<SourceFile> | undefined {
  try {
...
export function factory(program: Program): TransformerFactory<SourceFile> {
  ...
  // Angular v9 downlevel constructor transformer
  let transformerFactory = tryLoadV9Transformer(program);
